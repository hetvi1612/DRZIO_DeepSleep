...
public class Fragment_Level3 extends Fragment {
    ...
    private int a = 0;
    private int b = 0;
    private Adapter_Fragment3 adapter;
    ...
    ArrayList<Integer> mTempday = new ArrayList<>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        this.v = inflater.inflate(R.layout.fragment_levelthree, container, false);
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    tinydb.putInt(Constants.ALLDAYSPROGRESS_LV3KEY, (int) value);
                    tinydb.putString(Constants.DAYSLEFT_LV3KEY, daysleft);
...
//        new Daydatatask(getContext()).execute();
//        new CountTask().execute();
    ...
    public String Thumburl(Context context,TypedArray videoarray, int pos) {
        int[] resIds = new int[videoarray.length()];
        for (int i2 = 0; i2 < videoarray.length(); i2++) {
            resIds[i2] = videoarray.getResourceId(i2, -1);
        }
    ...
    public boolean isdayended(String name) {
        if (databaseOperations.getIsdaycomplete(name, 3) == 1) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean isPreviouscompleted() {
        ...
        for (int i = 0; i < 28; i++) {
            String name = "Day" + i;
            if (databaseOperations.getIsdaycomplete(name, 2) == 1) {
                count++;
            }
        }
        if (count == 24) {
    ...
    public float dayprogress(String name, int position) {
        try {
            if (isdayended(name)) {
                ...
                long mLong = databaseOperations.getExcDayProgress(name, 3) / 1000;
    ...
    private void bindDataToAdapter() {
        adapter = new Adapter_Fragment3(getContext(), items, weekdata,isLastdone);
    ...
    public void onResume() {
        super.onResume();
        try {
            if (tinydb.getString(Constants.ALV3DAYCLICK_KEY).equals("")) {
                if (adapter != null) {
                    if (adapter!= null){
